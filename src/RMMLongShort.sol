...
import {TokenIssuer, BasicToken} from "./TokenIssuer.sol";
...
contract RMMLongShort is ERC20 {
        ...
        evolve(index)
    {
        ...
        lastImpliedPrice = timeToExpiry > 0
            ? uint256(
                int256(computeSpotPrice(index.syToAsset(reserveX), totalLiquidity, strike, sigma, lastTau())).lnWad()
                    * int256(365 * 86400) / timeToExpiry
            )
            : 1 ether;
    }
